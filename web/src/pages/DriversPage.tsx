...
export function DriversPage() {
  ...
  const [statsLoadingIds, setStatsLoadingIds] = useState<Set<number>>(new Set());
  ...
  const filteredDrivers = useMemo(
    () => filterDrivers(drivers, query, statusFilter),
  ...
  useEffect(() => {
    const driversOnPage = paginateItems(filteredDrivers, safePage);
    if (driversOnPage.length === 0) return;

    let cancelled = false;
    const ids = driversOnPage.map((driver) => driver.id);
    setStatsLoadingIds(new Set(ids));

    Promise.all(
      ids.map(async (driverId) => {
        try {
          const summary = await getEarningsSummary({ driver_id: driverId });
          return {
            driverId,
            stats: {
              totalTrips: summary.total_trips,
              totalEarnings: summary.total_earnings ?? summary.estimated_income,
            },
          };
        } catch {
          return { driverId, stats: { totalTrips: 0, totalEarnings: 0 } };
        }
      })
    ).then((results) => {
      if (cancelled) return;
      setStatsMap((prev) => {
        ...
        for (const item of results) {
          next[item.driverId] = item.stats;
        }
        return next;
      });
      setStatsLoadingIds(new Set());
    });
  ...
  const rows: DriverRow[] = pageDrivers.map((driver) => ({
    ...
    statsLoading: statsLoadingIds.has(driver.id),
  }));
    ...
    <section className="wide-section">
      <div className="page-header">
        <div>
          ...
          <p className="muted">Общая база с мобильным приложением.</p>
        </div>
          ...
          <DataTable<DriverRow>
            ...
            columns={[
              ...
              },
              {
                ...
                header: 'Статус',
                render: (row) => (
                  <span className={row.is_active ? 'badge badge-success' : 'badge badge-muted'}>
                    {driverStatusLabel(row.is_active)}
                  </span>
                ),
              },
              {
                key: 'trips',
                header: 'Рейсы',
                render: (row) => (row.statsLoading ? '…' : String(row.stats.totalTrips)),
              },
              {
                key: 'earnings',
                header: 'Заработок',
                render: (row) =>
                  row.statsLoading ? '…' : formatMoney(row.stats.totalEarnings),
              },
              {
                key: 'actions',
                header: '',
